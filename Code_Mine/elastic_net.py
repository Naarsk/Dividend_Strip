...
valid_classes = ['Private Equity', 'Venture Capital', 'Real Estate']
asset_classes = [ac for ac in merged['AssetClass'].dropna().unique() if ac in valid_classes]

for asset_class in asset_classes:
    df_ac = merged[merged['AssetClass'] == asset_class]
    fund_ids = df_ac['FundID'].unique()

    for fund_id in fund_ids:
        df_fund = df_ac[df_ac['FundID'] == fund_id].copy()
        ...
        # Store data
        structured_data[asset_class][fund_id]['X'] = X
        structured_data[asset_class][fund_id]['y'] = y
...
for asset_class in structured_data:
    for fund_id, data in structured_data[asset_class].items():
        X = data['X']
        y = data['y']
        ...
        # CV folds cannot exceed num obs
        cv_folds = min(10, n_obs)
